...
type PendingScheduledTaskHandlingRequest = {
...
export function PlanningView({
  ...
  const [kanbanProjectFilterIds, setKanbanProjectFilterIds] =
  ...
  const [
    pendingScheduledTaskHandlingRequest,
    setPendingScheduledTaskHandlingRequest,
  ] = useState<PendingScheduledTaskHandlingRequest | null>(null);
  ...
  const dailyPlannerProjectFilterIds = useMemo(
    () => (kanbanProjectFilterIds === null ? null : new Set(kanbanProjectFilterIds)),
    [kanbanProjectFilterIds],
  );
  const dailyPlannerTasks = useMemo(() => {
    if (!dailyPlannerProjectFilterIds) {
      return plannerData.tasks;
    }

    return plannerData.tasks.filter((task) =>
      dailyPlannerProjectFilterIds.has(task.projectId ?? task.project?.id ?? ""),
    );
  }, [dailyPlannerProjectFilterIds, plannerData.tasks]);
  const dailyPlannerTaskIds = useMemo(
    () => new Set(dailyPlannerTasks.map((task) => task.id)),
    [dailyPlannerTasks],
  );
  const dailyPlannerDependencies = useMemo(
    () =>
      dailyPlannerTasks.flatMap((task) =>
  ...
  const dailyPlannerProjects = useMemo(() => {
    if (!dailyPlannerProjectFilterIds) {
      return plannerData.projects;
    }

    return plannerData.projects.filter((project) =>
      dailyPlannerProjectFilterIds.has(project.id),
    );
  }, [dailyPlannerProjectFilterIds, plannerData.projects]);
  const dailyPlannerMilestones = useMemo(() => {
    if (!dailyPlannerProjectFilterIds) {
      return plannerData.milestones;
    }

    return plannerData.milestones.filter((milestone) =>
      dailyPlannerProjectFilterIds.has(milestone.projectId),
    );
  }, [dailyPlannerProjectFilterIds, plannerData.milestones]);
  const dailyPlannerProjectPlans = useMemo(() => {
    if (!dailyPlannerProjectFilterIds) {
      return plannerData.projectPlans;
    }

    return plannerData.projectPlans.filter((plan) =>
      dailyPlannerProjectFilterIds.has(plan.project.id),
    );
  }, [dailyPlannerProjectFilterIds, plannerData.projectPlans]);
  const futureScheduledTasks = useMemo(
    () =>
      plannerData.scheduledItems.filter((item) => {
        if (item.source !== "task" || !item.taskId || item.status === "done") {
          return false;
        }

        if (!dailyPlannerTaskIds.has(item.taskId)) {
          return false;
        }

        if (dateInTimezone(item.start, plannerData.settings.timezone) !== focusedDate) {
          return false;
        }
    ...
    [
      dailyPlannerTaskIds,
      focusedDate,
      plannerData.scheduledItems,
      plannerData.settings.timezone,
    ],
  );
  ...
  const buildDailyPlanRequest = useCallback(
    (
      planningMode: PlanningMode,
      custom?: CustomPlanningPreferences,
      scheduledTaskHandling: ScheduledTaskHandling = "preserve_future",
    ): DailyPlanRequest => ({
      planningMode,
      scheduledTaskHandling,
      date: focusedDate,
      ...
      projects: dailyPlannerProjects,
      milestones: dailyPlannerMilestones,
      projectPlans: dailyPlannerProjectPlans,
      scheduledItems: plannerData.scheduledItems,
      capacity: plannerData.capacity,
      settings: plannerData.settings,
      customInstructions: custom,
      dependencies: dailyPlannerDependencies,
    }),
    [
      dailyPlannerDependencies,
      dailyPlannerMilestones,
      dailyPlannerProjectPlans,
      dailyPlannerProjects,
      dailyPlannerTasks,
      focusedDate,
      plannerData.capacity,
      plannerData.scheduledItems,
  ...
  const generateDailyPlan = useCallback(
    async (
      ...
      if (!scheduledTaskHandling && futureScheduledTasks.length) {
        setPendingScheduledTaskHandlingRequest({ planningMode, custom });
        return;
      }

      setPendingScheduledTaskHandlingRequest(null);
      setPlanLoading(true);
        ...
        const response = await fetch("/api/ai/daily-plan", {
          ...
          body: JSON.stringify(
            buildDailyPlanRequest(
              planningMode,
              custom,
              scheduledTaskHandling ?? "preserve_future",
            ),
          ),
        });
    ...
    [buildDailyPlanRequest, futureScheduledTasks.length],
  );
  ...
  const kanbanProjectOptionIds = useMemo(
    () => kanbanProjectOptions.map((project) => project.id),
    [kanbanProjectOptions],
  );

  useEffect(() => {
    if (kanbanProjectFilterIds === null) {
      return;
    }

    const validProjectIds = new Set(kanbanProjectOptionIds);
    const nextProjectIds = kanbanProjectFilterIds.filter((projectId) =>
      validProjectIds.has(projectId),
    );

    if (nextProjectIds.length !== kanbanProjectFilterIds.length) {
      setKanbanProjectFilterIds(nextProjectIds);
    }
  }, [kanbanProjectFilterIds, kanbanProjectOptionIds]);

  const selectedKanbanProjectIds = useMemo(
    () =>
      kanbanProjectFilterIds === null
        ? new Set(kanbanProjectOptionIds)
        : new Set(kanbanProjectFilterIds),
    [kanbanProjectFilterIds, kanbanProjectOptionIds],
  );

  const kanbanProjectFilterLabel = useMemo(() => {
    if (kanbanProjectFilterIds === null) {
      return "All projects";
    }

    if (kanbanProjectFilterIds.length === 0) {
      return "No projects";
    }

    if (kanbanProjectFilterIds.length === 1) {
      const project = kanbanProjectOptions.find(
        (candidate) => candidate.id === kanbanProjectFilterIds[0],
      );

      return project?.name ?? "1 project";
    }

    return `${kanbanProjectFilterIds.length} projects`;
  }, [kanbanProjectFilterIds, kanbanProjectOptions]);

  function toggleKanbanProjectFilter(projectId: string, checked: boolean) {
    setKanbanProjectFilterIds((currentProjectIds) => {
      const nextProjectIds = new Set(
        currentProjectIds === null ? kanbanProjectOptionIds : currentProjectIds,
      );
      ...
      const orderedProjectIds = kanbanProjectOptionIds.filter((candidateId) =>
        nextProjectIds.has(candidateId),
      );

      if (
        kanbanProjectOptionIds.length > 0 &&
        orderedProjectIds.length === kanbanProjectOptionIds.length
      ) {
        return null;
      }
  ...
  const filteredKanbanTasks = useMemo(() => {
    ...
    return tasks.filter(
      (task) =>
        selectedKanbanProjectIds.has(task.projectId ?? "") ||
        selectedKanbanProjectIds.has(task.project?.id ?? ""),
    );
  }, [kanbanProjectFilterIds, selectedKanbanProjectIds, tasks]);
    ...
    <div className="relative flex h-full flex-1 flex-col bg-[var(--background)]">
      ...
      <div className="relative min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
        ...
        >
          ...
          {pendingScheduledTaskHandlingRequest ? (
            <div
              data-testid="ai-planner-scheduled-task-choice"
              className="mt-3 rounded-xl border border-[var(--accent-strong)] bg-[var(--accent-soft)] p-3"
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[var(--foreground-strong)]">
                    Keep already scheduled tasks in place?
                  </p>
                  <p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">
                    {futureScheduledTasks.length} unfinished task
                    {futureScheduledTasks.length === 1 ? " is" : "s are"} scheduled later on{" "}
                    {formatFocusedDate(focusedDate)}. Keep those slots booked, or let the
                    planner rearrange them with the rest of the day.
                  </p>
                  <p className="mt-1 truncate text-xs text-[var(--muted-foreground)]">
                    {futureScheduledTasks
                      ...
                      .map((item) => item.title)
                      .join(", ")}
                    {futureScheduledTasks.length > 3 ? "..." : ""}
                  </p>
                </div>
                <div className="flex shrink-0 flex-wrap gap-2">
                  <button
                    type="button"
                    data-testid="ai-planner-preserve-scheduled"
                    onClick={() =>
                      generateDailyPlan(
                        pendingScheduledTaskHandlingRequest.planningMode,
                        pendingScheduledTaskHandlingRequest.custom,
                        "preserve_future",
                      )
                    }
                    disabled={planLoading}
                  ...
                  <button
                    type="button"
                    data-testid="ai-planner-rearrange-scheduled"
                    onClick={() =>
                      generateDailyPlan(
                        pendingScheduledTaskHandlingRequest.planningMode,
                        pendingScheduledTaskHandlingRequest.custom,
                    ...
                    disabled={planLoading}
                    className="h-9 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-semibold text-[var(--foreground-strong)] transition-colors hover:bg-[var(--surface-muted)] disabled:opacity-50"
                  >
                    Rearrange them
                  </button>
                </div>
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button
                      ...
                      className="ai-planner-apply-button h-8 rounded-lg px-3 text-xs font-semibold transition-colors disabled:opacity-50"
                    >
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      data-testid="task-flow-project-filter-trigger"
                      aria-label="Filter Task Flow by project"
                      className="inline-flex h-9 min-w-[11rem] max-w-[18rem] items-center justify-between gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-left text-sm font-medium text-[var(--foreground-strong)] shadow-sm outline-none transition-colors hover:bg-[var(--surface-muted)] focus-visible:border-[var(--accent-strong)] focus-visible:ring-2 focus-visible:ring-[var(--accent-soft)]"
                    >
                      <span className="truncate">{kanbanProjectFilterLabel}</span>
                      <ChevronDown className="h-4 w-4 shrink-0 text-[var(--muted-foreground)]" />
                    </button>
                  </PopoverTrigger>
                  <PopoverContent
                    align="start"
                    className="w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2 text-[var(--foreground)] shadow-[var(--shadow-float)]"
                  >
                    <div
                      data-testid="task-flow-project-filter-menu"
                      className="grid max-h-72 gap-1 overflow-y-auto"
                      aria-label="Task Flow project filters"
                    >
                      <label className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-sm font-medium text-[var(--foreground-strong)] transition-colors hover:bg-[var(--surface-muted)]">
                        <Checkbox
                          checked={kanbanProjectFilterIds === null}
                          onChange={(event) =>
                            setKanbanProjectFilterIds(event.target.checked ? null : [])
                          }
                        />
                        <span className="truncate">All projects</span>
                      </label>
                      <div className="my-1 h-px bg-[var(--border)]" />
                      {kanbanProjectOptions.length ? (
                        kanbanProjectOptions.map((project) => (
                          <label
                            key={project.id}
                            className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-sm text-[var(--foreground-strong)] transition-colors hover:bg-[var(--surface-muted)]"
                          >
                            <Checkbox
                              checked={selectedKanbanProjectIds.has(project.id)}
                              onChange={(event) =>
                                toggleKanbanProjectFilter(project.id, event.target.checked)
                              }
                            />
                            <span
                        ...
                        <p className="px-2 py-2 text-sm text-[var(--muted-foreground)]">
                          No projects yet.
                        </p>
                      )}
                    </div>
